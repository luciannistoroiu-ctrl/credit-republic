"""
WAVE•PM Top 5 Crypto Optimizer with CoinDesk MCP

Automatic data fetching from CoinDesk for:
- Real-time market data
- Multi-timeframe analysis (1d, 4h, 1h, 15m)
- Parameter optimization across 5 top cryptos

Fallback:
- Local TradingView CSV exports
- Cached data

Usage:
    python optimize_top5_multitf_coindesk.py
"""
...
from data_fetcher_coindesk import DataFetcherCoinDesk
...
class MultiTFCryptoOptimizer:
    """Optimize WAVE•PM parameters with CoinDesk MCP data."""
    ...
    def __init__(self):
        self.fetcher = DataFetcherCoinDesk()
    ...
    def optimize_symbol(self, symbol: str) -> list:
        ...
        print(f"\n{'=' * 70}")
        print(f"🔍 Optimizing {symbol} (Multi-Timeframe + CoinDesk MCP)")
        print(f"{'=' * 70}")
        ...
        print("Data sources: CoinDesk MCP → TradingView CSV → Cache")
        print("-" * 70)
    ...
    def save_report(self):
        ...
        report = {
            ...
            'type': 'Multi-Timeframe Optimization with CoinDesk MCP',
            'timeframes': self.TIMEFRAMES,
            'timeframe_weights': self.WEIGHTS,
            'timeframe_periods': self.TIMEFRAME_PERIODS,
            'best_per_crypto': self.best_params_per_crypto,
            'data_source': 'CoinDesk MCP (with TradingView CSV & cache fallback)',
            'top5_cryptos': self.TOP5_CRYPTOS
        }

        filename = f"top5_multitf_coindesk_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def run(self):
        ...
        print("\n" + "=" * 70)
        print("🚀 WAVE•PM Top 5 Crypto Optimizer with CoinDesk MCP")
        print("=" * 70)
        print(f"\nData Source: CoinDesk MCP (Auto-Sync)")
        ...
        # Show CoinDesk status
        if self.fetcher.has_coindesk:
            print(f"\n✓ CoinDesk MCP is connected and active!")
        else:
            print(f"\n⚠️  CoinDesk MCP not connected - using fallback sources")
            print(f"   To enable: https://claude.ai/customize/connectors")
